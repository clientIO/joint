...
QUnit.module('joint.mvc.Collection', function(hooks) {
    ...
    QUnit.test('add model to collection with sort()-style comparator', function(assert) {
        ...
        collection.add(rob);
        collection.add(tim);
        assert.equal(Array.from(collection).indexOf(rob), 0);
        assert.equal(Array.from(collection).indexOf(tim), 1);
        assert.equal(Array.from(collection).indexOf(tom), 2);
    });
    ...
    QUnit.test('reset passes caller options', function(assert) {
        ...
        assert.equal(collection.length, 2);
        Array.from(collection).forEach(function(model) {
