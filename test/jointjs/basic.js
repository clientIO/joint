QUnit.module('basic', function(hooks) {
    ...
    QUnit.test('cell.getAncestors()', function() {
        ...
        deepEqual(_.map(r2.getAncestors(), 'id'), [r1.id], 'A cell embedded in a parent with no ancestor has exactly one ancestor.');
        deepEqual(_.map(r5.getAncestors(), 'id'), [r2.id, r1.id], 'If a cell has more than one ancestor, the ancesotrs are sorted from the parent to the most distant ancestor.');
    });
