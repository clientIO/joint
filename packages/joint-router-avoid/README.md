...
- `routeAll()` - one-shot: routes every cell currently in the graph, resolving once all routes are applied. No graph listener is attached. Throws while started. Resolves with a `RoutingResult` - `{ status: 'done', routedLinks }` (the links whose route was applied, with their `origin`/`reason` - see `RoutedLink` below), or `{ status: 'cancelled' }` if the pass was interrupted by `destroy()`. Overlapping calls are queued and run one after another.
...
- `routeAllSync()` / `routeSubgraphSync(cells)` - synchronous counterparts for the main-thread provider: every route is on its link by the time they return, and errors throw out of the call. Throw with a Worker provider, while started, after `destroy()`, or while a `routeAll()`/`routeSubgraph()` pass is still in flight (`await` it first). Emit no routing events; instead they return what `link:routed` would have reported - an array of `{ link, origin, reason? }` (`RoutedLink`), one per link whose route was applied, in application order (links claimed via `interceptUnroutableLink` are not included).
