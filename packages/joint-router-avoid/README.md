...
- `routeAllSync()` / `routeSubgraphSync(cells)` - synchronous counterparts for the main-thread provider: every route is on its link by the time they return, and errors throw out of the call. Throw with a Worker provider, while started, after `destroy()`, or while a `routeAll()`/`routeSubgraph()` pass is still in flight (`await` it first). Return nothing and emit no routing events - the pass either completed or threw; use `setRouteAttributes`/`interceptUnroutableLink` for per-link hooks.
