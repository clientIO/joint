...
PortData.prototype = {
  ...
  getGroupPortsMetrics: function (groupName, elBBox) {
    ...
    util.toArray(groupPortTransformations).reduce(
      function (res, portTransformation, index) {
...
export const elementPortPrototype = {
  ...
  insertPort: function (before, port, opt) {
    const index =
      typeof before === "number" ? before : this.getPortIndex(before);
  ...
  _createPortData: function () {
    ...
    if (prevPortData) {
      var added = curPortData.filter(function (item) {
        if (
          !prevPortData.find(function (prevPort) {
      ...
      var removed = prevPortData.filter(function (item) {
        if (
          !curPortData.find(function (curPort) {
...
export const elementViewPortPrototype = {
  ...
  _createPortElement: function (port) {
    ...
    if (Array.isArray(labelMarkupDef)) {
      ...
      if (childCount > 0) {
        ...
        labelElement =
          childCount === 1 ? V(fragment.firstChild) : V("g").append(fragment);
      }
    } else {
      // String Markup
    ...
    for (var i = 0, n = portsMetrics.length; i < n; i++) {
      ...
      if (labelTransformation && cached.portLabelElement) {
        this.updateDOMSubtreeAttributes(
          cached.portLabelElement.node,
          labelTransformation.attrs,
          {
  ...
  _getPortMarkup: function (port) {
    return (
      port.markup ||
      this.model.get("portMarkup") ||
      this.model.portMarkup ||
      this.portMarkup
    );
  },
