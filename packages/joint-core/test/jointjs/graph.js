QUnit.module('graph', function(hooks) {
    ...
    this.setupTestMixtureGraph = function(graph) {
        ...
        a.embed(aa);
        var aaa = new joint.shapes.standard.Rectangle({ id: 'aaa' }).addTo(
            graph,
        ); // top, aa -> aaa -> top; aaa -> aaa (loop); child of a(aa)
        aa.embed(aaa);
        var top = new joint.shapes.standard.Rectangle({ id: 'top' }).addTo(
            graph,
        ); // aaa -> top -> aaa
        new joint.shapes.standard.Link({
            id: 'l10',
    ...
    QUnit.module('syncCells()', function(hooks) {
        ...
        QUnit.module('remove: false', function() {
        ...
        QUnit.module('remove: true', function() {
            ...
            QUnit.test('should synchronize cell attributes', function(assert) {
                ...
                graph.syncCells([
                    { id: 'a', type: 'standard.Rectangle', test: 'new-a' },
                    { id: 'b', type: 'standard.Rectangle', test: 'new-b' },
                    new joint.shapes.standard.Rectangle({ id: 'c' })
                ], { remove: false });
                const a = graph.getCell('a');
                assert.equal(a.get('test'), 'new-a');
            ...
            QUnit.test('should do all operations in batch mode', function(assert) {
                ...
                assert.ok(batchStartSpy.calledOnce, 'Batch start called once');
                assert.ok(batchStopSpy.calledOnce, 'Batch stop called once');
                // batch:start
                assert.ok(batchStartSpy.calledWith(sinon.match({ batchName: 'sync-cells' })));
                // batch:stop
                assert.ok(batchStopSpy.calledWith(sinon.match({ batchName: 'sync-cells' })));
            });
            ...
            QUnit.test('should preserve the connected links when replacing cells', function(assert) {
                ...
                graph.resetCells([
                    { id: 'a', type: 'standard.Rectangle' },
                    { id: 'b', type: 'standard.Rectangle' },
                    { id: 'c', type: 'standard.Rectangle' },
                    { id: 'l1', type: 'standard.Link', source: { id: 'a' }, target: { id: 'b' }},
                    { id: 'l2', type: 'standard.Link', source: { id: 'b' }, target: { id: 'c' }}
                ]);
                graph.syncCells([
                    { id: 'a', type: 'standard.Rectangle' },
                    { id: 'b', type: 'standard.Circle' }, // replace b with different type
                    { id: 'c', type: 'standard.Circle' }, // replace c with different type
                    { id: 'l1', type: 'standard.Link', source: { id: 'a' }, target: { id: 'b' }},
                    { id: 'l2', type: 'standard.Link', source: { id: 'b' }, target: { id: 'c' }}
                ], { remove: true });
                // assert that the links are still connected to the correct source/target cells
                const l1 = graph.getCell('l1');
                assert.equal(l1.getSourceCell().id, 'a');
                assert.equal(l1.getTargetCell().id, 'b');
                const l2 = graph.getCell('l2');
                assert.equal(l2.getSourceCell().id, 'b');
                assert.equal(l2.getTargetCell().id, 'c');
                // assert that the elements and links are preserved
                assert.equal(graph.getElements().length, 3);
                assert.equal(graph.getLinks().length, 2);
            });

            QUnit.test('should pass options to each operation', function(assert) {
                const graph = this.graph;
                assert.expect(4);
                const opt = { some: 'option', remove: true };
                graph.on('add', function(cell, _, options) {
                    assert.equal(options.some, 'option', 'add: Option value is correct');
                });
                graph.on('change', function(cell, options) {
                    assert.equal(options.some, 'option', 'change: Option value is correct');
                });
                graph.on('remove', function(cell, _, options) {
                    assert.equal(options.some, 'option', 'remove: Option value is correct');
                });
                graph.on('sort', function(collection, options) {
                    assert.equal(options.some, 'option', 'sort: Option value is correct');
                });
                graph.resetCells([
    ...
    QUnit.test('storing reference on models', function(assert) {
        ...
        assert.ok(
            a.graph === null,
            'The graph reference is nulled when the model is removed from the graph.',
        );
        ...
        assert.ok(
            b.graph === graph1,
            'The graph reference is not nulled when the model is removed from a graph which is not its owner.',
        );
        graph1.resetCells([]);
        assert.ok(
            b.graph === null,
    ...
    QUnit.test('dry flag', function(assert) {
        ...
        assert.ok(
            b.graph == null,
            'The graph reference is not stored after element added when the `dry` flag is passed.',
        );

        graph1.resetCells([b], { dry: true });
        assert.ok(
            b.graph == null,
            'The graph reference is not stored after graph reset when the `dry` flag is passed.',
        );
    ...
    QUnit.test('graph.clear()', function(assert) {
        ...
        var l1 = new joint.shapes.standard.Rectangle({
            source: { id: r1.id },
            target: { id: r2.id },
        });
        var l2 = new joint.shapes.standard.Rectangle({
            source: { id: r2.id },
            target: { id: r3.id },
        });
        var l3 = new joint.shapes.standard.Rectangle({
            source: { id: r2.id },
            target: { id: r4.id },
        });
        ...
        assert.equal(
            graph.getCells().length,
            0,
            'all the links and elements (even embeddes) were removed.',
        );
        assert.equal(
            graph.get('cells').length,
            0,
            'collection length is exactly 0.',
        );
    });

    QUnit.test(
        'graph.getCells(), graph.getLinks(), graph.getElements()',
        function(assert) {
            var graph = this.graph;
    ...
    QUnit.test('graph.getCommonAncestor()', function(assert) {
        ...
        assert.ok(
            !this.graph.getCommonAncestor(),
            'r1 embeds r2 and r3. r2 embeds r4 and r5. r3 embeds r6. r1 and r7 have no parents. Calling getCommonAncestor() returns no common ancestor.',
        );
        assert.equal(
            (this.graph.getCommonAncestor(r2) || {}).id,
            r1.id,
            'Common ancestor for r2 is r1.',
        );
        assert.equal(
            (this.graph.getCommonAncestor(r2, r3) || {}).id,
            r1.id,
            'Common ancestor for r2 and r3 is r1.',
        );
        assert.equal(
            (this.graph.getCommonAncestor(r2, r3, r4) || {}).id,
            r1.id,
            'Common ancestor for r2, r3 and r4 is r1',
        );
        assert.notOk(
            this.graph.getCommonAncestor(r2, r3, r7),
            'There is no common ancestor for r2, r3 and r5',
        );
        assert.notOk(
            this.graph.getCommonAncestor(r2, r3, r1),
            'There is no common ancestor for r2, r3 and r1',
        );
        assert.equal(
            (this.graph.getCommonAncestor(r5, r4) || {}).id,
            r2.id,
            'Common ancestor for r5 and r4 is r2',
        );
        assert.equal(
            (this.graph.getCommonAncestor(r5, r6) || {}).id,
            r1.id,
            'Common ancestor for r5 and r6 is r1',
        );
    });
    ...
    QUnit.test('graph.getConnectedLinks()', function(assert) {
        ...
        // SHALLOW LINKS:
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('a')), 'id')),
            _.sortBy(['l1', 'l2', 'l3', 'l6']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('a'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            ...
            _.sortBy(['l1', 'l2', 'l3']),
            'getConnectedLinks() returns all the connected links from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
            ...
            _.sortBy(['l6']),
            'getConnectedLinks() returns all the connected links to an element.',
        );

        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('b')), 'id')),
            _.sortBy(['l1', 'l5']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('b'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('c')), 'id')),
            _.sortBy(['l2']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('c'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('d')), 'id')),
            _.sortBy(['l3', 'l4']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('d'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('e')), 'id')),
            _.sortBy(['l4', 'l5', 'l6']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('e'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('f')), 'id')),
            _.sortBy(['l7']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('f'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('g')), 'id')),
            _.sortBy(['l8']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('g'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('h')), 'id')),
            _.sortBy([]),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('h'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(_.map(graph.getConnectedLinks(graph.getCell('aa')), 'id')),
            _.sortBy(['l10', 'l14']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('aa'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(
                _.map(graph.getConnectedLinks(graph.getCell('aaa')), 'id'),
            ),
            _.sortBy(['l11', 'l12', 'l13', 'l14']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('aaa'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            ...
            _.sortBy(['l11', 'l13', 'l14']),
            'getConnectedLinks() returns all the connected links to an element.',
        );

        assert.deepEqual(
            _.sortBy(
                _.map(graph.getConnectedLinks(graph.getCell('top')), 'id'),
            ),
            _.sortBy(['l10', 'l11', 'l12']),
            'getConnectedLinks() returns all the connected links to/from an element.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('top'), {
                        inbound: true,
                        outbound: true,
                    }),
                    'id',
        ...
        assert.deepEqual(
            _.sortBy(
                _.map(
                    graph.getConnectedLinks(graph.getCell('a'), { deep: true }),
                    'id',
                ),
            ),
            _.sortBy(['l1', 'l2', 'l3', 'l6', 'l10', 'l11', 'l12']),
            'deep getConnectedLinks() returns all the connected links to/from an element, except enclosed links.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
        ...
        assert.deepEqual(
            ...
            _.sortBy(['l1', 'l2', 'l3', 'l12']),
            'deep getConnectedLinks() returns all the connected links from an element, except enclosed links.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
            ...
            _.sortBy(['l6', 'l10', 'l11']),
            'deep getConnectedLinks() returns all the connected links to an element, except enclosed links.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
        ...
        assert.deepEqual(
            ...
            'deep getConnectedLinks() returns all the connected links to/from an element, including enclosed links.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
        ...
        assert.deepEqual(
            ...
            _.sortBy(['l1', 'l2', 'l3', 'l12', 'l13', 'l14']),
            'deep getConnectedLinks() returns all the connected links from an element, including enclosed links.',
        );
        assert.deepEqual(
            _.sortBy(
                _.map(
            ...
            _.sortBy(['l6', 'l10', 'l11', 'l13', 'l14']),
            'deep getConnectedLinks() returns all the connected links to an element, including enclosed links.',
        );
    ...
    QUnit.test(
        'graph.getSources(), graph.getSinks(), isSource(), isSink()',
        function(assert) {
            var graph = this.graph;
            this.setupTestMixtureGraph(graph);

            assert.deepEqual(
                _.sortBy(_.map(graph.getSources(), 'id')),
                _.sortBy(['f', 'h']),
                'getSources() returns all the root elements of the graph.',
            );
            assert.deepEqual(
                _.sortBy(_.map(graph.getSinks(), 'id')),
                _.sortBy(['b', 'c', 'g', 'h']),
                'getSinks() returns all the leaf elements of the graph.',
            );
            assert.equal(
                graph.isSink(graph.getCell('c')),
                true,
                'isSink() returns true for a root element.',
            );
            assert.equal(
                graph.isSink(graph.getCell('a')),
                false,
                'isSink() returns false for a non-root element.',
            );
            assert.equal(
                graph.isSource(graph.getCell('f')),
                true,
                'isSource() returns true for a leaf element.',
            );
            assert.equal(
                graph.isSource(graph.getCell('a')),
                false,
                'isSource() returns false for a non-leaf element.',
            );
            ...
            assert.equal(
                sources[0].id,
                'el1',
                'that one source is what we expect',
            );
            ...
            assert.equal(
                sources.length,
                2,
                'now we have two sources in the graph',
            );
            assert.deepEqual(
                _.map(sources, 'id').sort(),
                ['el1', 'el2'],
                'both sources are what we expect',
            );
        },
    );
    ...
    QUnit.test('graph.getSuccessors(), graph.isSuccessor()', function(assert) {
        ...
        assert.deepEqual(
            _.map(graph.getSuccessors(graph.getCell('b')), 'id'),
            ['e', 'n', 'f', 'o', 'g', 'p', 'c', 'h', 'i', 'j'],
            'getSuccessors() returns successors of an element in DFS order by default',
        );
        assert.deepEqual(
            _.map(
                graph.getSuccessors(graph.getCell('b'), { breadthFirst: true }),
                'id',
            ),
            ['e', 'f', 'g', 'c', 'n', 'o', 'p', 'h', 'i', 'j'],
            'getSuccessors() returns successors of an element in BFS order if breadthFirst option is true',
        );
        assert.deepEqual(
            _.map(graph.getSuccessors(graph.getCell('n')), 'id'),
            [],
            'getSuccessors() returns an empty array for a leaf element',
        );
        assert.equal(
            graph.isSuccessor(graph.getCell('b'), graph.getCell('o')),
            true,
            'isSuccessor() returns true if the element in second argument is a successor of the element in the first argument',
        );
        assert.equal(
            graph.isSuccessor(graph.getCell('b'), graph.getCell('a')),
            false,
            'isSuccessor() returns false if the element in second argument is not a successor of the element in the first argument',
        );
        assert.equal(
            graph.isSuccessor(graph.getCell('b'), graph.getCell('b')),
            false,
            'isSuccessor() returns false if the element in second argument is the same as the element in the first argument',
        );
    });

    QUnit.test(
        'graph.getPredecessors(), graph.isPredecessor()',
        function(assert) {
            var graph = this.graph;
            ...
            assert.deepEqual(
                _.map(graph.getPredecessors(graph.getCell('g')), 'id'),
                ['b', 'a', 'h', 'c'],
                'getPredecessors() returns predecessors of an element in DFS order by default',
            );
            assert.deepEqual(
                _.map(graph.getPredecessors(graph.getCell('a')), 'id'),
                [],
                'getPredecessors() returns an empty array for a root element',
            );
            assert.equal(
                graph.isPredecessor(graph.getCell('g'), graph.getCell('c')),
                true,
                'isPredecessor() returns true if the element in second argument is a successor of the element in the first argument',
            );
            assert.equal(
                graph.isPredecessor(graph.getCell('g'), graph.getCell('e')),
                false,
                'isPredecessor() returns false if the element in second argument is not a successor of the element in the first argument',
            );
            assert.equal(
                graph.isPredecessor(graph.getCell('g'), graph.getCell('g')),
                false,
                'isPredecessor() returns false if the element in second argument is the same as the element in the first argument',
            );
        },
    );
    ...
    QUnit.test('graph.bfs(), graph.dfs()', function(assert) {
        ...
        graph.bfs(
            graph.getCell('c'),
            function(element, distance) {
                bfs.push([element.id, distance]);
            },
            { outbound: true },
        );
        dfs = [];
        ...
        assert.deepEqual(
            ...
            'bfs() returns elements in a correct order with correct distance',
        );
        assert.deepEqual(
            dfs,
            [
            ...
            'dfs() returns elements in a correct order with correct distance',
        );
        ...
        assert.deepEqual(
            ...
            'bfs() correctly stopped when iteratee returned false',
        );
        assert.deepEqual(
            dfs,
            [
            ...
            'dfs() correctly stopped when iteratee returned false',
        );
    });
    ...
    QUnit.test('graph.cloneCells()', function(assert) {
        ...
        var clones = graph.cloneCells(
            [graph.getCell('d')].concat(
                graph.getSuccessors(graph.getCell('d')),
            ),
        );
        assert.deepEqual(
            _.map(clones, function(c) {
                return c.get('name');
            }),
            ['d', 'k', 'l', 'm'],
            'cloneCells() returns cloned elements without connected links',
        );
        assert.ok(_.isObject(clones), 'returned clone map is an object');
        assert.equal(
            clones['d'].get('name'),
            'd',
            'returned clone map maps original ID to the clone',
        );
        assert.notEqual(
            clones['d'].id,
            'd',
    ...
    QUnit.test('graph.cloneSubgraph()', function(assert) {
        ...
        var clones = graph.cloneSubgraph(
            [graph.getCell('d')].concat(
                graph.getSuccessors(graph.getCell('d')),
            ),
        );
        assert.deepEqual(
            ...
            ['d', 'k', 'l', 'm', 'l10', 'l11', 'l12'],
            'cloneSubgraph() returns cloned elements including connected links',
        );
    });
    ...
    QUnit.module('graph.getSubgraph()', function() {
        QUnit.test('link-element', function(assert) {
            ...
            var subgraph = graph.getSubgraph(
                [graph.getCell('d')].concat(
                    graph.getSuccessors(graph.getCell('d')),
                ),
            );
            assert.deepEqual(
                _.map(subgraph, 'id'),
                ['d', 'k', 'l', 'm', 'l10', 'l11', 'l12'],
                'getSubgraph() returns elements including links that are connected to any element passed as argument',
            );
            ...
            assert.deepEqual(
                _.map(subgraph, 'id'),
                ['a'],
                'getSubgraph() returns only the one element if deep is false',
            );
            ...
            assert.deepEqual(
                _.map(subgraph, 'id'),
                ['a', 'aa', 'l2', 'aaa', 'c', 'l1'],
                'getSubgraph() returns all the embedded elements and all the links that connect these elements',
            );
        });
    ...
    QUnit.module('graph.getCellsBBox()', function() {
        QUnit.test('sanity', function(assert) {
            ...
            assert.equal(
                this.graph.getCellsBBox([]),
                null,
                'graph.getBBox([]) with empty array returns null',
            );
            ...
            assert.deepEqual(
                ...
                'graph.getBBox() with link returns correct bounding box',
            );
        });
    ...
    QUnit.module('graph.findElementsUnderElement()', function() {
        QUnit.test('sanity', function(assert) {
            ...
            assert.deepEqual(
                this.graph.findElementsUnderElement(away),
                [],
                'There are no models under the element.',
            );
            assert.deepEqual(
                this.graph.findElementsUnderElement(rect),
                [under],
                'There is a model under the element.',
            );
            ...
            assert.deepEqual(
                ...
                [],
                'There is no model under the element if searchBy origin option used.',
            );
            assert.deepEqual(
                this.graph.findElementsUnderElement(rect, {
                    searchBy: 'corner',
                ...
                [under],
                'There is a model under the element if searchBy corner options used.',
            );
            ...
            assert.deepEqual(
                this.graph.findElementsUnderElement(rect),
                [under],
                'There is 1 model under the element found and 1 embedded element is omitted.',
            );
            assert.deepEqual(
                this.graph.findElementsUnderElement(under),
                [rect, embedded],
                'There are 2 models under the element. Parent and its embed.',
            );
            assert.deepEqual(
                this.graph.findElementsUnderElement(embedded),
                [rect, under],
                'There are 2 models under the element. The element\'s parent and one other element.',
            );
        });
    ...
    QUnit.module('graph.getNeighbors(), graph.isNeighbor()', function() {
        ...
        QUnit.test(
            'graph.getNeighbors(), graph.isNeighbor()',
            function(assert) {
                var graph = this.graph;
                ...
                assert.deepEqual(
                    neighbors(graph, r4),
                    [],
                    'Returns an empty array if the element has no neighbors.',
                );
                assert.deepEqual(
                    neighbors(graph, r1),
                    ['R2'],
                    'Element has only outbound link. The neighbor was found.',
                );
                assert.deepEqual(
                    neighbors(graph, r3),
                    ['R2'],
                    'Element has only inbound link. The neighbor was found.',
                );
                assert.deepEqual(
                    neighbors(graph, r2),
                    ['R1', 'R3'],
                    'Element has both outbound an inbound links. The neighbors were found.',
                );
                assert.equal(
                    graph.isNeighbor(r2, r3),
                    true,
                    'isNeighbor() returns true if the element in the second argument is a neighbor of the element in the first argument',
                );
                assert.equal(
                    graph.isNeighbor(r3, r2),
                    true,
                    'isNeighbor() returns true if the element in the second argument is a neighbor of the element in the first argument',
                );
                assert.equal(
                    graph.isNeighbor(r1, r3),
                    false,
                    'isNeighbor() returns true if the element in the second argument is not a neighbor of the element in the first argument',
                );
                assert.equal(
                    graph.isNeighbor(r3, r1),
                    false,
                    'isNeighbor() returns false if the element in the second argument is not a neighbor of the element in the first argument',
                );
                assert.equal(
                    graph.isNeighbor(r2, r3, { outbound: true }),
                    true,
                    'isNeighbor() with outbound true',
                );
                assert.equal(
                    graph.isNeighbor(r3, r2, { outbound: true }),
                    false,
                    'isNeighbor() with outbound true',
                );
                assert.equal(
                    graph.isNeighbor(r2, r3, { inbound: true }),
                    false,
                    'isNeighbor() with inbound true',
                );
                assert.equal(
                    graph.isNeighbor(r3, r2, { inbound: true }),
                    true,
                    'isNeighbor() with inbound true',
                );
                ...
                assert.deepEqual(
                    neighbors(graph, r2, { inbound: true }),
                    ['R1'],
                    'The inbound links were found.',
                );
                assert.deepEqual(
                    neighbors(graph, r2, { outbound: true }),
                    ['R3', 'R4'],
                    'The outbound links were found.',
                );
                ...
                assert.deepEqual(
                    neighbors(graph, r1),
                    ['R1'],
                    'Being a self-neighbor is detected.',
                );
                assert.equal(
                    graph.isNeighbor(r1, r1),
                    true,
                    'Being a self-neighbor is detected in isNeighbor().',
                );
                ...
                assert.deepEqual(
                    neighbors(graph, r1),
                    ['R3', 'R5'],
                    'Embedded elements are not taken into account by default.',
                );
                assert.deepEqual(
                    neighbors(graph, r2),
                    ['R4', 'R6'],
                    'Parent elements are not taken into account by default.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true }),
                    ['R3', 'R4', 'R5', 'R6'],
                    'The neighbours of the element and all its embdes were found in the deep mode. But not the embdes themselves.',
                );
                assert.deepEqual(
                    neighbors(graph, r2, { deep: true }),
                    ['R4', 'R6'],
                    'Parent elements are not taken into account in the deep mode.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true, outbound: true }),
                    ['R3', 'R4'],
                    'The outbound neighbours of the element and all its embdes were found in the deep mode.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true, inbound: true }),
                    ['R5', 'R6'],
                    'The inbound neighbours of the element and all its embdes were found in the deep mode.',
                );
                assert.equal(
                    graph.isNeighbor(r1, r4, { deep: true }),
                    true,
                    'isNeighbor() with deep true takes into account embedded elements',
                );
                assert.equal(
                    graph.isNeighbor(r1, r4, { deep: true, inbound: true }),
                    false,
                    'isNeighbor() with inbound true and deep true takes into account embedded elements',
                );
                ...
                assert.deepEqual(
                    neighbors(graph, r1),
                    ['R2'],
                    'A connected embedded elements is found in the shallow mode.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true }),
                    ['R1', 'R2'],
                    'All the connected embedded elements are found in the deep mode.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true, inbound: true }),
                    ['R1'],
                    'All the inbound connected embedded elements are found in the deep mode.',
                );
                assert.deepEqual(
                    neighbors(graph, r1, { deep: true, outbound: true }),
                    ['R2'],
                    'All the outbound connected embedded elements are found in the deep mode.',
                );
            },
        );
        ...
        QUnit.test('getNeighbor(), isNeighbor(), link-link', function(assert) {
            ...
            //               --l4--
            //
            assert.deepEqual(neighbors(graph, r1, { indirect: true }), [
                'R2',
                'R3',
            ]);
            assert.deepEqual(
                neighbors(graph, r1, { indirect: true, inbound: true }),
                [],
            );
            assert.deepEqual(
                neighbors(graph, r1, { indirect: true, outbound: true }),
                ['R3'],
            );

            assert.ok(graph.isNeighbor(r1, r3, { indirect: true }));
            assert.ok(
                graph.isNeighbor(r1, r3, { indirect: true, outbound: true }),
            );
            assert.notOk(
                graph.isNeighbor(r1, r3, { indirect: true, inbound: true }),
            );

            assert.deepEqual(neighbors(graph, r2, { indirect: true }), [
                'R1',
                'R2',
                'R3',
            ]);
            assert.deepEqual(
                neighbors(graph, r2, { indirect: true, inbound: true }),
                ['R2'],
            );
            assert.deepEqual(
                neighbors(graph, r2, { indirect: true, outbound: true }),
                ['R2', 'R3'],
            );

            assert.deepEqual(neighbors(graph, r3, { indirect: true }), [
                'R1',
                'R2',
                'R4',
            ]);
            assert.deepEqual(
                neighbors(graph, r3, { indirect: true, inbound: true }),
                ['R1', 'R2'],
            );
            assert.deepEqual(
                neighbors(graph, r3, { indirect: true, outbound: true }),
                ['R4'],
            );

            assert.deepEqual(neighbors(graph, r4, { indirect: true }), ['R3']);
            assert.deepEqual(
                neighbors(graph, r4, { indirect: true, inbound: true }),
                ['R3'],
            );
            assert.deepEqual(
                neighbors(graph, r4, { indirect: true, outbound: true }),
                [],
            );

            assert.deepEqual(neighbors(graph, l1, { indirect: true }), [
                'R1',
                'R2',
                'R3',
            ]);
            assert.deepEqual(
                neighbors(graph, l1, { indirect: true, inbound: true }),
                ['R1'],
            );
            assert.deepEqual(
                neighbors(graph, l1, { indirect: true, outbound: true }),
                ['R3'],
            );

            assert.deepEqual(neighbors(graph, l2, { indirect: true }), [
                'R1',
                'R2',
                'R3',
            ]);
            assert.deepEqual(
                neighbors(graph, l2, { indirect: true, inbound: true }),
                ['R1', 'R2'],
            );
            assert.deepEqual(
                neighbors(graph, l2, { indirect: true, outbound: true }),
                ['R3'],
            );

            assert.deepEqual(neighbors(graph, l3, { indirect: true }), [
                'R3',
                'R4',
            ]);
            assert.deepEqual(
                neighbors(graph, l3, { indirect: true, inbound: true }),
                ['R3'],
            );
            assert.deepEqual(
                neighbors(graph, l3, { indirect: true, outbound: true }),
                ['R4'],
            );

            assert.deepEqual(neighbors(graph, l4, { indirect: true }), ['R2']);
            assert.deepEqual(
                neighbors(graph, l4, { indirect: true, inbound: true }),
                ['R2'],
            );
            assert.deepEqual(
                neighbors(graph, l4, { indirect: true, outbound: true }),
                ['R2'],
            );
        });
    });
    ...
    QUnit.module('findElementsInArea(rect[, opt])', function(hooks) {
        ...
        QUnit.test('rect is instance of g.rect', function(assert) {
            ...
            assert.equal(
                modelsInArea.length,
                1,
                '[opt.strict = TRUE] should require elements to be completely within rect',
            );
        });
    ...
    QUnit.test('translate(dx, dy, opt)', function(assert) {
        ...
        assert.equal(
            after.rect1.bbox.x,
            before.rect1.bbox.x + dx,
            'position/x of element',
        );
        assert.equal(
            after.rect1.bbox.y,
            before.rect1.bbox.y + dy,
            'position/y of element',
        );
        assert.equal(
            after.link1.source.x,
            before.link1.source.x + dx,
            'source/x of link',
        );
        assert.equal(
            after.link1.source.y,
            before.link1.source.y + dy,
            'source/y of link',
        );
        assert.equal(
            after.link1.target.x,
            before.link1.target.x + dx,
            'target/x of link',
        );
        assert.equal(
            after.link1.target.y,
            before.link1.target.y + dy,
            'target/y of link',
        );
        ...
        assert.equal(
            after.embeddedElement1.bbox.x,
            before.embeddedElement1.bbox.x + dx,
            'position/x of embedded element',
        );
        assert.equal(
            after.embeddedElement1.bbox.y,
            before.embeddedElement1.bbox.y + dy,
            'position/y of embedded element',
        );
        assert.equal(
            after.embeddedLink1.source.x,
            before.embeddedLink1.source.x + dx,
            'source/x of embedded link',
        );
        assert.equal(
            after.embeddedLink1.source.y,
            before.embeddedLink1.source.y + dy,
            'source/y of embedded link',
        );
        assert.equal(
            after.embeddedLink1.target.x,
            before.embeddedLink1.target.x + dx,
            'target/x of embedded link',
        );
        assert.equal(
            after.embeddedLink1.target.y,
            before.embeddedLink1.target.y + dy,
            'target/y of embedded link',
        );
    });
    ...
    QUnit.module('resize()', function(hooks) {
        ...
        QUnit.test('increase size', function(assert) {
            ...
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 600, 500).toString(),
            );
            assert.equal(
                g.point(this.l2.get('vertices')[0]).toString(),
                g.point(500, 500).toString(),
            );
        });
        ...
        QUnit.test('decrease size', function(assert) {
            ...
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 60, 50).toString(),
            );
            assert.equal(
                g.point(this.l2.get('vertices')[0]).toString(),
                g.point(140, 140).toString(),
            );
        });
    });
    ...
    QUnit.module('resizeCells()', function(hooks) {
        ...
        QUnit.test('no cells provided', function(assert) {
            ...
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 300, 250).toString(),
            );
            assert.equal(
                g.point(this.l2.get('vertices')[0]).toString(),
                g.point(300, 300).toString(),
            );
        });

        QUnit.test('elements and links provided', function(assert) {
            var graph = this.graph;
            graph.resizeCells(100, 100, [
                this.ea,
                this.eb,
                this.ec,
                this.l1,
            ...
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 100, 100).toString(),
            );
            graph.resizeCells(300, 400, graph.getCells());
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 300, 400).toString(),
            );
            assert.equal(
                g.point(this.l2.get('vertices')[0]).toString(),
                g.point(300, 420).toString(),
            );
        });
        ...
        QUnit.test('only elements provided', function(assert) {
            ...
            assert.equal(
                graph.getBBox().toString(),
                g.rect(100, 100, 200, 250).toString(),
            );
            assert.equal(
                graph.getCellsBBox([this.ea, this.eb]).toString(),
                g.rect(100, 100, 100, 100).toString(),
            );
        });
    ...
    QUnit.module('graph.fromJSON()', function() {
        ...
        QUnit.test('z attribute should inherit correctly', function(assert) {
            ...
            assert.equal(
                this.graph.getCell('some-cell').get('z'),
                joint.shapes.Custom.prototype.defaults.z,
                '',
            );
    ...
    QUnit.module('graph.toJSON()', function(hooks) {
        ...
        QUnit.test(
            'should return graph data as JSON object',
            function(assert) {
        ...
        QUnit.test(
            'should take in account `opt.ignoreDefaults` = false',
            function(assert) {
                const rect = new joint.shapes.standard.Rectangle();

                this.graph.resetCells([rect]);
                const json = this.graph.toJSON({
                    cellAttributes: { ignoreDefaults: false },
                });
        ...
        QUnit.test(
            'should take in account `opt.ignoreDefaults` = false, `opt.ignoreEmptyAttributes` = true',
            function(assert) {
        ...
        QUnit.test(
            'should take in account `opt.ignoreDefaults` = true',
            function(assert) {
                const rect = new joint.shapes.standard.Rectangle();

                this.graph.resetCells([rect]);
                const json = this.graph.toJSON({
                    cellAttributes: { ignoreDefaults: true },
                });

                assert.deepEqual(json.cells[0], {
                    type: joint.shapes.standard.Rectangle.prototype.defaults
                        .type,
                    id: rect.id,
                    size: {},
                    position: {},
        ...
        QUnit.test(
            'should take in account `opt.ignoreDefaults` = true, `opt.ignoreEmptyAttributes` = true',
            function(assert) {
                const rect = new joint.shapes.standard.Rectangle();
        ...
        QUnit.test(
            '`opt.ignoreDefaults` should accept an array of keys to differentiate',
            function(assert) {
                const rect = new joint.shapes.standard.Rectangle();

                this.graph.resetCells([rect]);
                const json = this.graph.toJSON({
                    cellAttributes: { ignoreDefaults: ['attrs', 'size'] },
                });
    ...
    QUnit.module('graph.transferCellEmbeds()', function() {
        ...
        QUnit.test(
            'should transfer embeds from an element to a link',
            function(assert) {
    ...
    QUnit.module('graph.transferCellConnectedLinks()', function() {
        QUnit.test('should transfer links of an element', function(assert) {
            ...
            const link1 = new joint.shapes.standard.Link({
                source: { id: originalElement.id },
            });
            const link2 = new joint.shapes.standard.Link({
                target: { id: originalElement.id },
            });
            ...
            this.graph.transferCellConnectedLinks(
                originalElement,
                replacementElement,
            );
        ...
        QUnit.test('should transfer links of a link', function(assert) {
            ...
            const link1 = new joint.shapes.standard.Link({
                source: { id: originalLink.id },
            });
            const link2 = new joint.shapes.standard.Link({
                target: { id: originalLink.id },
            });
        ...
        QUnit.test(
            'should work when transferring links from a link to an element',
            function(assert) {
                const originalLink = new joint.shapes.standard.Link();
                const link1 = new joint.shapes.standard.Link({
                    source: { id: originalLink.id },
        ...
        QUnit.test(
            'should work when transferring links from an element to a link',
            function(assert) {
                const originalElement = new joint.shapes.standard.Rectangle();
                const link1 = new joint.shapes.standard.Link({
                    source: { id: originalElement.id },
    ...
    QUnit.module('layer API', function() {
        ...
        QUnit.test('layerAttribute config option', function(assert) {
            ...
            assert.equal(
                rect.get('_layerId'),
                'my-layer',
                'layer attribute is changed accordingly',
            );
            ...
            assert.equal(
                rect.layer(),
                'another-layer',
                'layer() reflects changes in the layer attribute',
            );
        ...
        QUnit.module('addLayer()', function() {
            ...
            QUnit.test('no options', function(assert) {
                ...
                assert.notOk(graph.hasLayer('my-layer'));

                graph.addLayer({ id: 'my-layer' });
                assert.ok(graph.hasLayer('my-layer'), '\'my-layer\' added');
            });
            ...
            QUnit.test('throwing errors', function(assert) {
                ...
                assert.throws(
                    ...
                    /Layer to add is invalid./,
                    'throws when adding a layer without id',
                );
        ...
        QUnit.module('moveLayer()', function() {
            ...
            QUnit.test('before option', function(assert) {
                const graph = new joint.dia.Graph(
                    {},
                    { cellNamespace: joint.shapes },
                );
                ...
                graph.addLayer({ id: 'layer3' });

                // Move at the end.
                graph.moveLayer('layer1', { before: null });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['cells', 'layer2', 'layer3', 'layer1'],
                    '\'layer1\' moved at the end',
                );
                ...
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['layer1', 'cells', 'layer2', 'layer3'],
                    '\'layer1\' moved to the beginning',
                );

                // Move in the middle.
                graph.moveLayer('layer1', { before: 'layer3' });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['cells', 'layer2', 'layer1', 'layer3'],
                    '\'layer1\' moved in the middle',
                );

            });

            QUnit.test('index option', function(assert) {
                const graph = new joint.dia.Graph(
                    {},
                    { cellNamespace: joint.shapes },
                );
                ...
                // Move to index 0.
                graph.moveLayer('layer2', { index: 0 });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['layer2', 'cells', 'layer1', 'layer3'],
                    '\'layer2\' moved to index 0',
                );

                // Move before the last index.
                ...
                graph.moveLayer('layer2', { index: 2 });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['cells', 'layer1', 'layer2', 'layer3'],
                    '\'layer2\' moved to middle index',
                );

                // Use negative index.
                graph.moveLayer('layer1', { index: -1 });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['layer1', 'cells', 'layer2', 'layer3'],
                    '\'layer1\' moved to the beginning using negative index',
                );

                // Move to last index using infinity.
                graph.moveLayer('layer1', { index: Infinity });
                assert.deepEqual(
                    graph.getLayers().map((layer) => layer.id),
                    ['cells', 'layer2', 'layer3', 'layer1'],
                    '\'layer1\' moved to the end using infinity',
                );
            });
            ...
            QUnit.test('throwing errors', function(assert) {
                ...
                assert.throws(
                    () => {
                        graph.moveLayer('non-existing-layer', { index: 0 });
                    },
                    /Layer to move does not exist./,
                    'throws when moving non-existing layer',
                );
                ...
                assert.throws(
                    () => {
                        graph.moveLayer('layer1', { before: 'layer2', index: 0 });
                    },
                    /Options "before" and "index" are mutually exclusive./,
                    'throws when both \'before\' and \'index\' options are specified',
                );
            });
            ...
            QUnit.test('events', function(assert) {
                ...
                graph.moveLayer('layer1', { index: 1, testOption: true });
                assert.notOk(eventSpy.called);

                graph.moveLayer('layer1', { index: 2, testOption: true });
                assert.ok(
                    eventSpy.calledWithMatch('layers:sort', sinon.match.any, { testOption: true }),
                    'options are passed to \'sort\' event'
                );
        ...
        QUnit.module('removeLayer()', function() {
            ...
            QUnit.test('throwing errors', function(assert) {
                ...
                assert.throws(
                    () => {
                        graph.removeLayer('cells');
                    },
                    /default layer cannot be removed./,
                    'throws when removing the default \'cells\' layer',
                );
            });
