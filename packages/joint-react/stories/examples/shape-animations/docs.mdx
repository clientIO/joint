...
`useOnPaperEvents()` listens to this custom event and updates the graph accordingly.
...
- **Prefer stable handlers**: In real apps, keep event handler references stable for predictable subscriptions.
