...
`Paper` accepts a `ref` and exposes its instance/context via that ref. The instance (including the underlying JointJS Paper) can be accessed via the `paperCtx` property on the ref object. (For graph access, use the `useGraph()` hook from inside the provider tree — there is no ref on `GraphProvider`.)
