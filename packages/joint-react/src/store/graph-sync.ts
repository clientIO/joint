...
import type { GraphStorePublicSnapshot } from './graph-store';
...
interface Options<Graph extends dia.Graph, Element extends GraphElement, Link extends GraphLink> {
  ...
  readonly store: ExternalStoreLike<GraphStorePublicSnapshot<Element, Link>>;
}
const BATCH_START_EVENT_NAME = 'batch:start';
const BATCH_STOP_EVENT_NAME = 'batch:stop';

/**
 * Synchronizes a JointJS graph with an external store, handling bidirectional updates
 * between React state and the graph while preventing circular update loops.
...
>(options: Options<Graph, Element, Link>): GraphSync {
  ...
  const onIncrementalChange = () => {
    ...
    store.setState((previous) => {
      ...
      // Elements
      const nextElements: GraphElement[] = [];
  ...
  const onBatchStop = (event: { batchName?: string }) => {
    batchCounter--;
