...
function defaultElementToAttributes({ id, element }: { id: string; element: MixedElementRecord }) {
  return elementToAttributes({ id, element });
}

function defaultLinkToAttributes({ id, link }: { id?: string; link: MixedLinkRecord }) {
  ...
  describe('updateGraph', () => {
    it('syncs elements to the graph', () => {
      ...
      controller.updateGraph({
        elements: {
          'el-1': { data: undefined, position: { x: 10, y: 20 }, size: { width: 100, height: 50 } } as MixedElementRecord<undefined>,
        },
        links: {},
      });
