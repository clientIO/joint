...
describe('use-elements', () => {
  ...
  it('should get elements properly without selector', async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.get('1')?.size.width).toBe(97);
      expect(result.current.get('1')?.size.height).toBe(99);

      expect(result.current.get('2')?.size.width).toBe(97);
      expect(result.current.get('2')?.size.height).toBe(99);
    });
  });
  ...
  it('should get elements properly with selector', async () => {
    ...
    const { result } = renderHook(
      () => {
        ...
        return useElements((elements) => [...elements.values()].map((item) => item.size.width));
      },
      {
        wrapper,
      }
