...
import { PAPER_ELEMENTS_SIZE_READY } from '../types/event.types';
...
function resolvePaper(
  ...
  if (typeof target === 'string') return null; // handled via paperById
...
export function useElementsMeasured(
  ...
  useEffect(() => {
    ...
    controller.listenTo(paper, PAPER_ELEMENTS_SIZE_READY, () => callbackRef.current());
