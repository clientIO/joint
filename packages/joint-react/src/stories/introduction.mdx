...
### 🔹 Accessing Elements
- {getAPIDocumentationLink('useElementsData')}: Retrieve all graph elements (requires `GraphProvider` context).
...
Elements and links are plain objects with geometry properties. They are stored as Records keyed by their ID.

```ts
const initialElements = {
  '1': { type: 'rect', x: 10, y: 10, width: 100, height: 100 },
};
