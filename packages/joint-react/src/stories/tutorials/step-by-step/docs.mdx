...
This example demonstrates how to use `@joint/react` in controlled mode with React's `useState` hook. In controlled mode, the graph state is managed in React state, and changes to the graph are synchronized back to your state.
...
3. **Bidirectional Sync**: Changes made via graph methods (like dragging nodes) automatically sync back to React state through the callbacks.
