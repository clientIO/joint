...
Elements are plain objects with at least an `id` and geometry properties (`x`, `y`, `width`, `height`).
...
Links are plain objects with `id`, `source`, and `target` properties.
...
1. **State Management**: Use `useState` to manage elements and links:
   ```tsx
