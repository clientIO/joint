...
Performance values are not stored in the graph — they are computed inline during render using `useElementsData()` selectors. When any input changes (funds, year, or percentages), React automatically re-renders the affected performance nodes with fresh calculations.

```tsx
const { value, roi } = useElementsData((elements) => {
...
Each diagram element uses SVG `foreignObject` to embed standard HTML form controls (inputs, selects) inside the SVG canvas. Changes are handled via React event handlers that update the graph store using `useGraph().set()`.
...
3. **User input**: Changing funds, year, or percentages calls `useGraph().set()` to update the graph store.
4. **Reactive computation**: Performance nodes use `useElementsData()` selectors to read inputs and compute values inline.
