...
Performance values are not stored in the graph — they are computed inline during render using `useElements()` selectors. When any input changes (funds, year, or percentages), React automatically re-renders the affected performance nodes with fresh calculations.
