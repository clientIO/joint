...
const elements: Record<string, AnyElementRecord<{ label: string }>> = {
...
function getPortDragElements(): Record<string, AnyElementRecord> {
  return {
    [SOURCE_ELEMENT_ID]: {
      data: {},
      position: { x: 40, y: 40 },
...
async function renderPortDragPaper(defaultLink?: DefaultLinkProperty) {
  ...
  let linksSnapshot: Map<string, AnyLinkRecord> = new Map();

  function CaptureLinksSnapshot() {
    linksSnapshot = useLinks() as unknown as Map<string, AnyLinkRecord>;
    return null;
  }
...
describe('Paper Component', () => {
  ...
  it('calls onElementsMeasured when element sizes change', async () => {
    const onMeasuredMock = jest.fn();
    const updatedElements: Record<string, AnyElementRecord<{ label: string }>> = {
  ...
  it('should set elements and positions via react state, when change it via paper api', async () => {
    ...
    let currentOutsideElements: Record<string, AnyElementRecord> = {};
    function Content() {
      const [currentElements, setCurrentElements] =
        useState<Record<string, AnyElementRecord>>(elementsWithPosition);
  ...
  it('should update elements via react state, and then reflect the changes in the paper', async () => {
    function Content() {
      const [currentElements, setCurrentElements] = useState<Record<string, AnyElementRecord>>(elements);
  ...
  describe('renderLink — useLink layout', () => {
    ...
    const testElements: Record<string, AnyElementRecord<{ label: string }>> = {
      'el-1': {
        data: { label: 'A' },
        position: { x: 50, y: 50 },
        size: { width: 100, height: 50 },
    ...
    const testLinks: Record<string, AnyLinkRecord> = {
