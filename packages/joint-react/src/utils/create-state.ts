...
/**
 * Options for creating a derived state from external stores.
 * @template TStates - The type of the source states
 * @template S - The type of the derived state
 */
interface DerivedStoreLike<TSnapshot = unknown> {
  readonly getSnapshot: () => TSnapshot;
  readonly subscribe: (listener: () => void) => () => void;
}

type DerivedSelectorArguments<TStates extends readonly DerivedStoreLike[]> = {
  readonly [K in keyof TStates]: ReturnType<TStates[K]['getSnapshot']>;
};

interface DerivedBaseOptions<S> {
...
interface DerivedStatesOptions<TStates extends readonly DerivedStoreLike[], S>
  extends DerivedBaseOptions<S> {
  /** The source external stores to derive from */
  readonly state: TStates;
  /** Function that extracts the desired portion from the source states */
  readonly selector: (...state: DerivedSelectorArguments<TStates>) => S;
}
/**
 * Creates a derived state from external stores using a selector function.
 * The derived state automatically updates when the selected portion of source states changes.
 ...
 * @returns A State instance that automatically syncs with the selected portion of source states
 */
export function derivedState<const TStates extends readonly DerivedStoreLike[], S>(
  options: DerivedStatesOptions<TStates, S>
): State<S> {
  const { state, isEqual = util.isEqual, name } = options;
  const stateSources = state as readonly DerivedStoreLike[];
  const getSnapshot = (): S => {
    const snapshots = stateSources.map((source) => source.getSnapshot());
    const selectMany = options.selector as (...values: DerivedSelectorArguments<TStates>) => S;
    return selectMany(...(snapshots as DerivedSelectorArguments<TStates>));
  };
  ...
  const listeners = stateSources.map((source) =>
    source.subscribe(() => {
