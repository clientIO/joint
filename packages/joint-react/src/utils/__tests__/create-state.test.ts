...
describe('createState', () => {
  ...
  describe('subscribers', () => {
    ...
    it('should not notify subscribers on silent update', () => {
      const subscriber = jest.fn();
      const state = createState({
        newState: () => ({ count: 0 }),
      });
      state.subscribe(subscriber);
      state.setState({ count: 1 }, true);
  ...
  describe('equality checks', () => {
    it('should use default equality (reference equality)', () => {
      const subscriber = jest.fn();
      const state = createState({
        newState: () => ({ count: 0 }),
      });
      state.subscribe(subscriber);
      const sameObject = { count: 0 };
      state.setState(sameObject);
      expect(subscriber).toHaveBeenCalledTimes(1);
      state.setState(sameObject);
      expect(subscriber).toHaveBeenCalledTimes(1);
  ...
  describe('computeAfterStateChange', () => {
    it('should apply computeAfterStateChange transformation', () => {
      const subscriber = jest.fn();
      const state = createState({
        newState: () => ({ count: 0 }),
        computeAfterStateChange: (previous) => ({
          ...previous,
      ...
      state.subscribe(subscriber);
      state.setState((previous) => ({ ...previous, count: previous.count + 1 }));
      expect(state.getSnapshot().count).toBe(2);
      expect(subscriber).toHaveBeenCalledTimes(1);
    });

    it('should use updated state in computeAfterStateChange', () => {
      const state = createState({
        newState: () => ({ count: 0, previous: 0 }),
        computeAfterStateChange: (updated) => ({
          ...updated,
          previous: updated.count,
        }),
      });
      state.setState({ count: 5, previous: 0 });
      const snapshot = state.getSnapshot();
      expect(snapshot.count).toBe(5);
      expect(snapshot.previous).toBe(5);
    });
  });
  ...
  describe('selectors', () => {
    it('should create selector that tracks selected state', () => {
      ...
      const selectorState = state.select((s) => s.count);
    ...
    it('should allow multiple selectors on same state', () => {
      ...
      const countSelector = state.select((s) => s.count);
      const nameSelector = state.select((s) => s.name);
    ...
    it('should handle nested selectors', () => {
      ...
      const userSelector = state.select((s) => s.user);
      const nameSelector = userSelector.select((u) => u.profile.name);
  ...
  describe('getAreComponentsNotified', () => {
    ...
    it('should return false after silent update', () => {
      const state = createState({
        newState: () => ({ count: 0 }),
      });
      state.setState({ count: 1 }, true);
