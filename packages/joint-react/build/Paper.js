...
export function Paper({
  ...
  useEffect(() => {
    ...
    const attributeChangeEvents = cellAttributes.map(attribute => `change:${attribute}`).join(' ');
    graphListener.listenTo(graph, attributeChangeEvents, cell => setElement(cell));
    paper.on(PORTAL_READY_EVENT, (elementView, portalEl) => setElement(elementView.model, portalEl));
    if (onReady) {
      onReady(paper);
    }
    if (onEvent) {
