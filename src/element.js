...
import { portsPrototype } from './ports';

export const Element = Cell.extend(Object.assign({
    ...
    translate: function(tx, ty, opt) {
        ...
        if (opt.transition) {
            ...
            if (!util.isObject(opt.transition)) opt.transition = {};

            this.transition('position', translatedPosition, util.assign({}, opt.transition, {
        ...
        util.invoke(this.getEmbeddedCells(), 'translate', tx, ty, opt);
    ...
    size: function(width, height, opt) {
        ...
        if (util.isObject(width)) {
            opt = height;
            height = util.isNumber(width.height) ? width.height : currentSize.height;
            width = util.isNumber(width.width) ? width.width : currentSize.width;
        }
    ...
    fitEmbeds: function(opt) {
        ...
        if (embeddedCells.length > 0) {
            ...
            if (opt.deep) {
                ...
                util.invoke(embeddedCells, 'fitEmbeds', opt);
            }
