import { dia, shapes, linkTools, elementTools, connectionStrategies, highlighters } from '@joint/core';
import { RoleGroup, Role, Lifeline, LifeSpan, Message } from './shapes';
import './styles.css';
...
paper.on('blank:pointerdblclick', function(evt, x, y) {
    const role = new Role({ position: { x: x - 50, y: topY }});
    role.addTo(graph);
...
const role1 = new Role({ position: { x: 100, y: topY }});
role1.setName('Browser');
role1.addTo(graph);

const role2 = new Role({ position: { x: 400, y: topY }});
role2.setName('Web Server');
role2.addTo(graph);

const role3 = new Role({ position: { x: 600, y: topY }});
role3.setName('Database Server');
role3.addTo(graph);

const backend = new RoleGroup();
backend.embed(role2);
backend.embed(role3);
backend.addTo(graph);
backend.fitRoles();
