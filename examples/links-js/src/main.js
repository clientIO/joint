import { dia, shapes, V, g, util, linkTools } from '@joint/core';
import './styles.css';
...
const link6 = new shapes.standard.DoubleLink({
    source: { x: 10, y: 200 },
    target: { x: 350, y: 200 },
    attrs: {
        line: {
...
const link8 = new shapes.standard.ShadowLink({
    source: { x: 10, y: 280 },
    target: { x: 440, y: 280 },
...
const el1 = new shapes.standard.Path({
...
const RectangleSourceArrowhead = linkTools.SourceArrowhead.extend({
    tagName: 'rect',
    attributes: {
        x: -15,
        y: -15,
...
const CircleTargetArrowhead = linkTools.TargetArrowhead.extend({
    tagName: 'circle',
    attributes: {
        r: 20,
        fill: 'black',
...
const CustomBoundary = linkTools.Boundary.extend({
    attributes: {
        fill: '#7c68fc',
        fillOpacity: 0.2,
        stroke: '#33334F',
...
paper.on('link:mouseenter', function(linkView) {
    ...
    switch (linkView.model) {
        ...
        case link6:
            tools = [
                new linkTools.Vertices({ vertexAdding: { interactiveLinkNode: 'outline' }}),
...
link10.findView(paper).addTools(new dia.ToolsView({
    name: 'permanent',
    tools: [
...
link13.findView(paper).addTools(new dia.ToolsView({
    name: 'permanent',
    tools: [
        new linkTools.TargetAnchor({
            restrictArea: false,
            snap: function(coords) {
                const bbox = this.model.getTargetCell().getBBox();
        ...
        new linkTools.SourceAnchor({
            restrictArea: false,
            snap: function(coords) {
                const bbox = this.model.getSourceCell().getBBox();
...
paper.on('myclick:circle', function(linkView, evt) {
    ...
    const transitionOpt = {
        delay: 100,
        duration: 2000,
        timingFunction: util.timing.inout
    };
