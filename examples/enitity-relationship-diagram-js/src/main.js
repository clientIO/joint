import { dia, shapes, g, V } from '@joint/core';
import './styles.css';
...
const Line = shapes.standard.Link.define('erd.Line', {
    attrs: {
        line: {
            stroke: '#ccc0ef',
            strokeWidth: 1,
...
ISA.prototype.getConnectionPoint = function(referencePoint) {
...
const paid = new IdentifyingRelationship({
...
const salesman = employee.clone().translate(0, 200).attr('text/text', 'Salesman');
const date = employeeName.clone().position(585, 80).attr('text/text', 'Date');
const car = employee.clone().position(430, 400).attr('text/text', 'Company car');
const plate = number.clone().position(405, 500).attr('text/text', 'Plate');

// Helpers

const createLink = function(elm1, elm2) {
    const myLink = new Line({
        source: { id: elm1.id },
        target: { id: elm2.id }
    });
